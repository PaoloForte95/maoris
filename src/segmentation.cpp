...
// void makeGraph(cv::Mat& slam, AASS::maoris::GraphZone& graph_slam, double&
// time){
//
...
// 	end_process = getTime();	decompose_time = end_process -
// begin_process; 	time = decompose_time;
//
...
// *****************/\n"; 	cv::Mat out_tmp_slam; 	AASS::maoris::ZoneExtractor
// zone_maker; 	zone_maker.addValueToIgnore(0);
//
// 	begin_process = getTime();
// 	AASS::maoris::reduceZone(out_slam, out_tmp_slam, 5);
// 	zone_maker.extract(out_tmp_slam);
// 	end_process = getTime();	decompose_time = end_process -
// begin_process; 	time = time + decompose_time;
//
...
//
// 	end_process = getTime();	decompose_time = end_process -
// begin_process; 	time = time + decompose_time; 	std::cout << "Ripples: " <<
// decompose_time << std::endl;
//
// 	begin_process = getTime();
// // 	graph_slam.updatePCA();
// 	graph_slam.updateContours();
...
cv::Mat threshold_maps(cv::Mat& slam_in_color) {
    ...
    while (not_good == true) {
        //		cv::Mat slam_in;
        //		cv::cvtColor(slam_in_color, slam_in,
        //cv::COLOR_BGR2GRAY); 	cv::imshow("map in", slam_in); 	cv::waitKey(0);
        //		slam = slam_in > step;

        cv::inRange(slam_in_color, cv::Scalar(step, 0, 0),
                    cv::Scalar(255, 255, 255), slam);
        ...
        //cv::BORDER_REPLICATE); Fill mask with value 128
        cv::floodFill(slam, seed, cv::Scalar(255), 0, cv::Scalar(),
                      cv::Scalar());
        cv::floodFill(slam, seed, cv::Scalar(0), 0, cv::Scalar(), cv::Scalar());

        cv::imshow("Threhsold", slam);
        auto output = cv::waitKey(0);
        std::cout << "OUTPUT " << output << "and step " << step << " and seed "
                  << seed.x << " " << seed.y << std::endl;
...
void process(cv::Mat& slam_in,
    ...
    // 	cv::resize(graphmat, graphmat, cv::Size(graphmat.cols * 2, graphmat.rows
    // * 2)); 	cv::imshow("GRAPH Visible", graphmat_vis);

    // 	std::cout << "Size of graph" << graph_slam.getNumVertices() <<
    // std::endl;
    ...
    cv::Mat graphmat_straight =
        AASS::maoris::segment_Ground_Truth(segmented_map);
...
std::tuple<std::string, std::vector<std::string> > getMapsAndGT() {
    std::string in = "o";
    std::cout << "Please input a source folder for the maps to segment"
              << std::endl;
    ...
    while (!boost::filesystem::exists(p) ||
        ...
        std::cout
            << "need a valid path toward the images - please add new input"
            << std::endl;
        std::getline(std::cin, in);
    ...
    std::cout
        << "Please input as many gt folder as you want. Input nothing if done."
        << std::endl;
    while (!gt.empty()) {
        ...
        if (!boost::filesystem::exists(p) ||
            !boost::filesystem::is_directory(p)) {
            std::cout << "need a valid path toward the images - ignoring input"
                      << std::endl;
        } else {
            if (gt.back() != '/') {
                gt = gt + "/";
...
int main(int argc, char** argv) {
    ...
    std::cout << "Welcome to the maoris test program.\n\n**** Press space to "
                 "run maoris on the webcam image.\n**** Press t, m, r, or d to "
                 "change the parameters\nInput: "
              << std::endl;

    process(frame, threshold, margin, ripples, doors);
